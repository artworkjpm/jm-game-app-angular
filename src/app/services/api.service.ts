...
export class AppService {
  ...
  CartoonChooser(searchItem: string): Observable<any> {
    const URL = urlPath + API_KEY + "&q=" + encodeURIComponent(searchItem) + "&image_type=illustration" + "&per_page=20";
    return this.httpClient.get(URL);
  }

  PhotoWinnerChooser(searchItem: string): Observable<any> {
    const URL = urlPath + API_KEY + "&q=" + encodeURIComponent(searchItem) + "&image_type=photo" + "&per_page=20";
    return this.httpClient.get(URL);
  }
}
